...
module.exports = async function(callback) {
  ...
  const saleHalflife = 700      // CRISP example sets to 700
  const priceSpeed = 1           // CRISP example sets to 1
  const priceHalflife = 700     // CRISP example sets to 100
  const startingPrice = 100    // Start price in ETH. Will be converted to wei in the script

  // Simulation variables
  const blocksToMine = 0     // number of additional blocks to mine between mint transactions EXAMPLE WAS 0
  const priceThreshold = 1000 // simulates the highest price a user is willing pay in ETH. If the price rises above this,
