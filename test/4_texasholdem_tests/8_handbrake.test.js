...
contract("TexasHoldemV1 - play", async function(accounts) {
  ...
  describe('game #1 - flop not dealt', function() {
    ...
    it( "game must exist to handbrake", async function () {
      expectRevert(
        this.texasHoldem.handbrake( gameId, { from: accounts[0] } ),
        "does not exist"
      )
    } )
    ...
    it( "only owner can handbrake", async function () {
      expectRevert(
        this.texasHoldem.handbrake( gameId, { from: accounts[1] } ),
        "Ownable: caller is not the owner"
      )
    })

    it( "can handbrake when flop deal failed", async function () {
  ...
  describe('game #2 - flop dealt, flop round not ended', function() {
    ...
    it( "can handbrake during flop round, and no hands played", async function () {
      const receipt = await this.texasHoldem.handbrake( gameId, { from: accounts[0] } )
      expectEvent(receipt, "GameDeleted", {
  ...
  describe('game #3 - flop dealt, flop round ended, no hands added to flop', function() {
    ...
    it( "can handbrake when flop round ended and no hands played", async function () {
      const receipt = await this.texasHoldem.handbrake( gameId, { from: accounts[0] } )
      expectEvent(receipt, "GameDeleted", {
        gameId: new BN(gameId),
      })
  ...
  describe('game #4 - flop dealt, hands added to flop, flop ended', function() {
    ...
    it( "can handbrake when flop round ended and hands played but game is not stale", async function () {
      const receipt = await this.texasHoldem.handbrake( gameId, { from: accounts[0] } )
    ...
    it("cannot request deal when in refundable state", async function() {
      expectRevert(
        this.texasHoldem.requestDeal(gameId, {from: accounts[0]}),
        "game refundable!"
      )
    })
  ...
  describe('game #5 - flop dealt, hands added to flop, turn requested but not fulfilled', function() {
    ...
    it( "can handbrake when turn was not fulfilled", async function () {
      const receipt = await this.texasHoldem.handbrake( gameId, { from: accounts[0] } )
      expectEvent(receipt, "RefundableGame", {
        gameId: new BN(gameId),
      })
  ...
  describe('game #6 - flop & turn dealt, no hands added to turn', function() {
  ...
  describe('game #7 - flop & turn played. Turn not ended', function() {
    const gameId = 7
    ...
    it( "can handbrake when turn has not ended yet, but hands are in play", async function () {
      const receipt = await this.texasHoldem.handbrake( gameId, { from: accounts[0] } )
      expectEvent(receipt, "RefundableGame", {
        gameId: new BN(gameId),
      })
  ...
  describe('game #8 - flop & turn played. Turn has ended', function() {
    const gameId = 8
    ...
    it("cannot request river deal when in refundable state", async function() {
      expectRevert(
        this.texasHoldem.requestDeal(gameId, {from: accounts[0]}),
        "game refundable!"
      )
  ...
  describe('game #9 - flop & turn played. River requested but never fulfilled', function() {
    const gameId = 9
    ...
    it( "can handbrake when river not fulfilled and game not yet stale", async function () {
      const receipt = await this.texasHoldem.handbrake( gameId, { from: accounts[0] } )
      expectEvent(receipt, "RefundableGame", {
        gameId: new BN(gameId),
      })
  ...
  describe('game #10 - flop, turn & river dealt, no final hands played, river round not ended yet', function() {
    const gameId = 10
  ...
  describe('game #11 - final game stage. Final hands played - CANNOT HANDBRAKE', function() {
    const gameId = 11

    before(async function() {

      // No need to populate PHE with data, as we're not calculating winners in this test
      // Just subscribe to phe
    ...
    it("player 1 adds a final hand", async function() {
      const tokenId = finalHands[0][0]
      const riverCards = finalHands[0][1]
      await this.texasHoldem.playFinalHand(tokenId, riverCards, gameId, {from: accounts[0]})
    })

    it( "cannot handbrake if at least one final hand has been played", async function () {
      expectRevert(
        this.texasHoldem.handbrake( gameId, { from: accounts[0] } ),
        "cannot handbrake when final hands exist"
      )
    })

    it("game should not be in refundable state", async function() {
      const game = await this.texasHoldem.games(gameId)
      expect(game.status).to.be.bignumber.not.eq(new BN(7))
    })

    it("game is not stale", async function() {
      const stale = await this.texasHoldem.gameIsStale( gameId )
      expect( stale ).to.be.eq( false )
    })

    it("player 2 adds a final hand", async function() {
      const tokenId = finalHands[1][0]
      const riverCards = finalHands[1][1]
      await this.texasHoldem.playFinalHand(tokenId, riverCards, gameId, {from: accounts[1]})
    })
    ...
    it("can end game as normal", async function() {
    ...
    it( "cannot handbrake after game ended", async function () {
      expectRevert(
        this.texasHoldem.handbrake( gameId, { from: accounts[0] } ),
        "does not exist"
      )
    ...
    it("game should not exist", async function() {
      const game = await this.texasHoldem.games(gameId)
      expect(game.status).to.be.bignumber.eq(new BN(0))
    })
